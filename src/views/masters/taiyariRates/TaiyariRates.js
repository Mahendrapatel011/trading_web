...
import { taiyariRateApi, masterApi, unitApi, locationApi } from '../../../api/reservationApi'
...
const TaiyariRates = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await taiyariRateApi.create(formData)
                setSuccess('Taiyari rate created successfully!')
            } else {
                await taiyariRateApi.update(currentRate.id, formData)
                setSuccess('Taiyari rate updated successfully!')
            }
            await loadData()
            setShowModal(false)
    ...
    const handleDelete = async (id) => {
        ...
        setLoading(true)
        try {
            await taiyariRateApi.delete(id)
    ...
    return (
        <CRow>
            ...
            <CModal visible={showModal} onClose={() => setShowModal(false)} size="lg">
                <CModalHeader><CModalTitle>{modalMode === 'add' ? 'Add Taiyari Rate' : 'Edit Taiyari Rate'}</CModalTitle></CModalHeader>
                <CModalBody>
                    <CForm>
                        <CRow className="mb-3">
                            <CCol md={6}>
