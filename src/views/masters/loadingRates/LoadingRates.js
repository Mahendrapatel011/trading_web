...
import { loadingRateApi, masterApi, unitApi, locationApi } from '../../../api/reservationApi'
...
const LoadingRates = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await loadingRateApi.create(formData)
                setSuccess('Loading rate created successfully!')
            } else {
                await loadingRateApi.update(currentRate.id, formData)
                setSuccess('Loading rate updated successfully!')
            }
            await loadData()
            setShowModal(false)
    ...
    const handleDelete = async (id) => {
        ...
        setLoading(true)
        try {
            await loadingRateApi.delete(id)
    ...
    return (
        <CRow>
            <CCol xs={12}>
                <CCard className="mb-4">
                    <CCardHeader className="d-flex justify-content-between align-items-center">
                        <strong>Loading Labour Rate Fixation</strong>
            ...
            <CModal visible={showModal} onClose={() => setShowModal(false)} size="lg">
                <CModalHeader><CModalTitle>{modalMode === 'add' ? 'Add Loading Rate' : 'Edit Loading Rate'}</CModalTitle></CModalHeader>
                <CModalBody>
                    <CForm>
                        <CRow className="mb-3">
                            <CCol md={6}>
