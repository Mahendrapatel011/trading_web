...
import { interestRateApi, locationApi } from '../../../api/reservationApi'
...
const InterestRates = () => {
    ...
    const loadData = async () => {
        setLoading(true)
        try {
            const [ratesRes, locationsRes] = await Promise.all([
                interestRateApi.getAllAdmin(),
                locationApi.getAll(),
            ])
    ...
    const handleSave = async () => {
        ...
        try {
            if (modalMode === 'add') {
                await interestRateApi.create(formData)
                setSuccess('Interest rate created successfully!')
            } else {
                await interestRateApi.update(currentRate.id, formData)
    ...
    const handleDelete = async (id) => {
        ...
        setLoading(true)
        try {
            await interestRateApi.delete(id)
    ...
    return (
        <CRow>
            <CCol xs={12}>
                <CCard className="mb-4">
                    <CCardHeader className="d-flex justify-content-between align-items-center">
                        <strong>Interest Rate Fixation</strong>
            ...
            <CModal visible={showModal} onClose={() => setShowModal(false)} size="lg">
                <CModalHeader><CModalTitle>{modalMode === 'add' ? 'Add Interest Rate' : 'Edit Interest Rate'}</CModalTitle></CModalHeader>
                <CModalBody>
                    <CForm>
                        <CRow className="mb-3">
                            <CCol md={6}>
                            ...
                            <CCol md={6}>
                                ...
                                <CFormInput type="number" name="rate" value={formData.rate} onChange={handleChange} required step="0.01" />
                            </CCol>
                        </CRow>
